...
// Admin email verify  
export const adminEmailVerify = async (req, res) => {
    ...
    try {
        ...
        const updatedUser = await AdminAccountModel.findByIdAndUpdate(
            decoded.adminId,
            { isVerified: true },
            { new: true }
        );
...
export const adminLoginAccount = async (req, res) => {
    ...
    try {
        ...
        const isAccount = await AdminAccountModel.findOne({ email, role });
